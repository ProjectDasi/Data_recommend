...
import pandas as pd
df = pd.read_csv('./visualize/data/job_merged_file_with_predictions.csv')
...
stopwords = [
    ...
    # 동사/형용사
    '가능', '필요', '원함', '바람', '포함', '선택', '희망', '진행', '완료', '시행', '유지', '관리', '운영', '점검', '대비', '준비', '조정', '변경', '수정', '접수', '완료', '수행', '처리', '필수',
    # 추가 불용어
...
reduced_count = dict(count.most_common(len(count) // 4))
