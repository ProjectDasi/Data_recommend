...
# 엑셀 파일 로드
work_info = pd.read_excel('./crawling_data_merged/job_transform_column_final_2(0826).xlsx')  # 회사 정보 파일의 경로를 올바르게 대체하세요
...
for index, row in work_info.iterrows():
    ...
    if not match_found:
        work_info.at[index, 'region_id'] = 1

# 새 열이 추가된 회사 정보를 저장
output_file_path = './data/job_merged_file_surplus_region(0826).csv'
work_info.to_csv(output_file_path, index=False, encoding='utf-8-sig')
